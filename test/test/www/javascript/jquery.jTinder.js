...
;(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		init: function (element) {
			...
			panes = $(">ul>li", element);
			paneID = ["one","two","three","four","five"];
		...
		showPane: function (index) {
			...
			if(index == 0)
			{
				...
				{
					panes.eq(x).show();
					panes.eq(x).animate({"transform": "translate(0px,0px) rotate(0deg)"}, $that.settings.animationRefreshSpeed);
					panes.eq(x).find($that.settings.likeSelector).animate({"opacity": 0}, $that.settings.animationRefreshpeed);
					panes.eq(x).find($that.settings.dislikeSelector).animate({"opacity": 0}, $that.settings.animationRefreshSpeed);
				}
				
				for(i = 0; i < 5; i++)
				{
					document.getElementById(paneID[i]).innerHTML ="<div class='img'> </div><div> Loading </div> <div class='like'> </div> <div class='dislike'> </div>";
				}
				...
				xmlhttp.onreadystatechange = function() {
					if (this.readyState == 4 && this.status == 200) {
						...
						{
							document.getElementById(paneID[i]).innerHTML ="<div class='img'> </div><div>"+ wishlistArr[i].name+"</div> <div class='like'> </div> <div class='dislike'> </div>";
						}
					}
				};
				xmlhttp.open("GET", "https://jwtristo.000webhostapp.com/list5resto.php", true);
				xmlhttp.send();
			}
			...
			switch (ev.type) {
				...
				case 'touchend':
					...
					if (opa >= 1.75) {
						...
						if(((Math.abs(deltaX) / $that.settings.threshold) / 100 + 0.2) < 0.3)
						{
							giveRestaurantDetails();
						}
	...
	function giveRestaurantDetails()
	{
		alert("Testing 123");
	}
